...
const Message = ({message, messages, index}) => {
    ...
    return (
            ...
            <div className="flex items-end py-0.5 px-1">
                ...
                <p style={{marginLeft : `${isSameSenderMargin(messages, message, index, user.user._id)}px`}} className={` text-[15px]  max-w-[400px] font-normal rounded-xl ${(message.sender._id === user.user._id) ? "bg-sky-400" :  "bg-green-400 "}  break-words h-full p-2`}>{message.content}</p>
            </div>
